...
export default function Enfermeria() {
  ...
  if (selected.emergency) {
    ...
    const updatePromises = hoyCitas
      ...
      const maxOrden = hoyCitas.reduce((max, c) => 
        c.orden_llegada > max ? c.orden_llegada : max
      , 0);
          ...
          {pendientes.length === 0
            ...
            : pendientes.map(cita => (
              <div 
                ...
                <p>{cita.motivo} - { new Date(cita.created_at).toLocaleString('es-MX',
                  {
                  hour12: true,
                  hour: '2-digit',
                  minute: '2-digit',
                ...
                />
                <button onClick={() => programarCita(cita.id)}>Programar</button>
              </div>
            ))
          }
          </div>
              ...
              <tbody>
                {programadas.map(cita => (
                  ...
                  >
                    ...
                    <td>{cita.motivo}</td>
                    <td>{new Date(cita.programmer_at).toLocaleDateString()}</td>
                    <td>{new Date(cita.programmer_at).toLocaleTimeString('es-MX', {
                      hour12: true,
                      hour: '2-digit',
                      minute: '2-digit'
                    })}</td>
                ...
                {enEspera
                  ...
                  .map(cita => (
                    ...
                    >
                      ...
                      <div className="card-turno">Turno: <span>#{cita.orden_llegada}</span></div>
